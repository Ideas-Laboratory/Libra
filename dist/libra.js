...
// src/query/query.js
var registeredSelectionManagers = {};
var instanceSelectionManagers = [];
...
SelectionManager.register = function register(name, optionOrSelectionManager) {
  ...
  if (optionOrSelectionManager instanceof SelectionManager) {
    ...
    option.constructor = option.hasOwnProperty("constructor") ? option.constructor : SelectionManager;
  }
...
SelectionManager.initialize = function initialize(name, ...params) {
  ...
  if (option = registeredSelectionManagers[name]) {
    const query2 = new option.constructor(name, ...option.extraParams || [], ...params);
    instanceSelectionManagers.push(query2);
    return query2;
  }
  return null;
};
...
// src/query/point.js
var PointSelectionManager = class extends SelectionManager {
  x = 0;
  y = 0;
  evaluate() {
    let result = [];
    this._layers.forEach((layer) => {
      result = result.concat(layer.pick({
        type: "point",
        x: this.x,
        y: this.y
      }));
...
// src/query/rect.js
var RectSelectionManager = class extends SelectionManager {
  x = 0;
  y = 0;
  width = 0;
  height = 0;
  evaluate() {
    if (!this._layers[0] || !this._layers[0].objects || !(this._layers[0].objects instanceof Function))
      return;
    const objects = this._layers[0].objects();
    let root3 = this._layers[0]._root;
    let bbox = { left: 0, top: 0 };
    if (root3) {
      root3 = root3.node ? root3.node() : root3;
      if (root3.getBoundingClientRect) {
        bbox = root3.getBoundingClientRect();
      }
    }
    const result = new Set();
    let x4 = this.width < 0 ? this.x - this.width : this.x;
    let y4 = this.height < 0 ? this.y - this.height : this.y;
    let width = Math.abs(this.width);
    let height = Math.abs(this.height);
    for (let i = x4; i < x4 + width; i++) {
      for (let j = y4; j < y4 + height; j++) {
        if (result.size === objects.length) {
          break;
        }
        let elements = document.elementsFromPoint(i + bbox.left, j + bbox.top);
        [...objects].forEach((obj) => {
          if (obj.node ? elements.includes(obj.node()) : elements.includes(obj)) {
            result.add(obj);
          }
        });
      }
...
// src/query/circle.js
var CircleSelectionManager = class extends SelectionManager {
  cx = 0;
  cy = 0;
  r = 0;
  evaluate() {
    ...
    const result = new Set();
    let x12 = this.cx - this.r;
    let y12 = this.cy - this.r;
    let x22 = this.cx + this.r;
    let y22 = this.cy + this.r;
    for (let i = x12; i < x22; i++) {
      for (let j = y12; j < y22; j++) {
        if (result.size === objects.length) {
          break;
        }
        if (Math.sqrt(Math.pow(i - this.cx, 2) + Math.sqrt(Math.pow(j - this.cy, 2))) > this.r)
          continue;
        let elements = document.elementsFromPoint(i + bbox.left, j + bbox.top);
        [...objects].forEach((obj) => {
          if (obj.node ? elements.includes(obj.node()) : elements.includes(obj)) {
...
// src/query/nothing.js
var NothingSelectionManager = class extends SelectionManager {
  evaluate() {
  }
};
...
// src/helpers.js
function makeFlexibleListener() {
  return new Proxy({ listener: {}, counter: {} }, {
    get(target, p) {
      if (p === Symbol.iterator)
        return Object.keys(target.listener)[Symbol.iterator];
      if (!target.listener[p])
        target.listener[p] = [];
      if (!target.counter[p])
        target.counter[p] = 0;
      return {
...
function makeComposableSelectionManager(self) {
  return new Proxy(self, {
    ...
    has(target, p) {
      if (!target.hasOwnProperty(p)) {
        return !!target.selectionManagers.find((selectionManager) => selectionManager.hasOwnProperty(p));
      } else {
        return true;
      }
    },
    getOwnPropertyDescriptor(target, p) {
      if (!target.hasOwnProperty(p)) {
        return target.selectionManagers.find((selectionManager) => selectionManager.hasOwnProperty(p)) ? { configurable: true, enumerable: true } : void 0;
      } else {
...
var FreezableMap = class extends Map {
  set(...args) {
...
// src/query/union.js
var UnionSelectionManager = class extends SelectionManager {
  selectionManagers = [];
  ...
  evaluate() {
    ...
    this.selectionManagers.forEach((selectionManager) => {
      selectionManager.result.forEach((x4) => {
        if (!result.includes(x4)) {
          result.push(x4);
        }
      });
    });
...
var IntersectionSelectionManager = class extends SelectionManager {
  selectionManagers = [];
  ...
  evaluate() {
    ...
    this.selectionManagers.forEach((selectionManager) => {
      if (!result)
        result = selectionManager.result;
      else
        result = selectionManager.result.filter((x4) => result.includes(x4));
    });
    this._result = result;
  }
};
...
var SubtractSelectionManager = class extends SelectionManager {
  selectionManagers = [];
  ...
  evaluate() {
    ...
    this.selectionManagers.forEach((selectionManager) => {
      if (!result)
        ...
        result = result.filter((x4) => !selectionManager.result.includes(x4));
    });
    this._result = result;
  }
};
...
// src/query/data.js
var DataSelectionManager = class extends SelectionManager {
  field = null;
  min = -Infinity;
  max = Infinity;
  evaluate() {
    ...
    let result = [];
    this._layers.forEach((layer) => {
      result = result.concat(layer.find((datum2) => datum2 && datum2[this.field] >= this.min && datum2[this.field] <= this.max));
    });
    this._result = [...new Set(result)];
  }
};
...
var Instrument = class {
  ...
  _injectInteractor(relation) {
    ...
    if (relation.const) {
      ...
      if (!this._interactors.includes(relation.interactor)) {
        ...
        [primitiveCommands, primitiveFeedbacks].flatMap((x4) => x4).forEach((type2) => {
          relation.interactor._listeners[type2].set(this._notify.bind(this, type2));
        });
      }
  ...
  _notify(type2, event) {
    if (!this._next)
      this._next = requestAnimationFrame(this._run.bind(this));
    if (this._eventQueue.find((pendingEvent) => pendingEvent.type === type2 && pendingEvent.event === event))
      return;
    this._eventQueue.push({ type: type2, event });
  }
  _run() {
    let pendingEvent = this._eventQueue.shift();
    while (pendingEvent) {
      let { type: type2, event } = pendingEvent;
      this._listeners[type2].list().forEach((listener) => {
...
Instrument.register = function register2(name, optionOrInstrument) {
  let option;
  if (optionOrInstrument instanceof Instrument) {
    ...
    option.constructor = option.hasOwnProperty("constructor") ? option.constructor : Instrument;
  }
  registeredInstruments[name] = option;
  return true;
};
Instrument.unregister = function unregister2(name) {
  delete registeredInstruments[name];
  return true;
};
Instrument.initialize = function initialize2(name, ...params) {
  let option;
...
function initWithOption(name, options, ...params) {
  const instrument = new options.constructor(name, ...options.extraParams || [], ...params);
  ...
  if (options.selectionManager || options.relations) {
    ...
    instrument.associate({
      selectionManager: options.selectionManager && options.selectionManager.clone(),
      relations: options.relations && options.relations.map((relation) => {
        if (relation.interactor) {
          let newInteractor;
  ...
  if (options.props) {
    Object.entries(options.props).forEach(([k2, v]) => {
      instrument.prop(k2, v);
    });
  }
...
var trajectoryInteractor = Interactor.initialize("TrajectoryInteractor");
...
Instrument.register("BrushInstrument", {
  ...
  relations: [
    ...
    },
    {
      ...
      interactor: trajectoryInteractor,
      dragCommand: (e, query2) => [e.x - query2.x, e.y - query2.y]
    }
  ]
});
var wheelInteractor = Interactor.initialize("WheelInteractor");
...
var pointerInteractor = Interactor.initialize("PointerInteractor");
  ...
  listen(options) {
    ...
    if (options.layers) {
      for (let layer of options.layers) {
        options.updateCommand && layer._listeners.unshift(options.updateCommand);
        options.updateFeedback && layer._listeners.push(options.updateFeedback);
      }
    } else if (options.layer) {
      options.updateCommand && options.layer._listeners.unshift(options.updateCommand);
      options.updateFeedback && options.layer._listeners.push(options.updateFeedback);
...
Layer.register = function register3(name, optionOrLayer) {
  let option;
  if (optionOrLayer instanceof Layer) {
    ...
    option.constructor = option.hasOwnProperty("constructor") ? option.constructor : Layer;
  }
  registeredLayers[name] = option;
  return true;
};
Layer.unregister = function unregister3(name) {
  delete registeredLayers[name];
  return true;
};
Layer.initialize = function initialize3(name, ...params) {
  let options;
  if (options = registeredLayers[name]) {
    const layer = new options.constructor(name, ...options.extraParams || [], ...params);
    if (options.preInstall && options.preInstall instanceof Function) {
...
function brush(dim) {
  ...
  function brush2(group2) {
    var overlay = group2.property("__brush", initialize5).selectAll(".overlay").data([type("overlay")]);
...
function collide_default(radius) {
  ...
  force.radius = function(_) {
    return arguments.length ? (radius = typeof _ === "function" ? _ : constant_default8(+_), initialize5(), force) : radius;
  };
  return force;
}
...
function link_default(links) {
  ...
  force.links = function(_) {
    return arguments.length ? (links = _, initialize5(), force) : links;
  };
  force.id = function(_) {
...
function manyBody_default() {
  ...
  force.strength = function(_) {
    return arguments.length ? (strength = typeof _ === "function" ? _ : constant_default8(+_), initialize5(), force) : strength;
  };
...
function x_default2(x4) {
  ...
  force.x = function(_) {
    return arguments.length ? (x4 = typeof _ === "function" ? _ : constant_default8(+_), initialize5(), force) : x4;
  };
  return force;
}
...
function y_default2(y4) {
  ...
  force.y = function(_) {
    return arguments.length ? (y4 = typeof _ === "function" ? _ : constant_default8(+_), initialize5(), force) : y4;
  };
  return force;
}
...
function ordinal() {
  var index3 = new Map(), domain = [], range3 = [], unknown = implicit;
  function scale(d) {
...
function pie_default() {
  ...
  function pie(data) {
    var i, n = (data = array_default4(data)).length, j, k2, sum3 = 0, index3 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da = Math.min(tau6, Math.max(-tau6, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da) / n, padAngle.apply(this, arguments)), pa = p * (da < 0 ? -1 : 1), v;
    for (i = 0; i < n; ++i) {
...
var D3Layer = class extends Layer {
  _root = create_default("svg:g");
  _width;
  _height;
  _checked = false;
  constructor(name = "D3Layer", width, height, svg2) {
    super(name);
    this._name = name;
    this._width = width;
    this._height = height;
    if (svg2) {
      this._root = svg2.append("g");
      this._container = svg2;
      this._checked = true;
    }
    this._root.append("rect").attr("class", "ig-layer-background").attr("width", this._width).attr("height", this._height).attr("opacity", 0);
  }
  _toTemplate() {
    return {
      ...super._toTemplate(),
  ...
  _dispatchEvent(event) {
    ...
    for (let wrappedEvent of wrappedEvents) {
      for (let instrument of this._pureInstruments) {
        instrument.dispatch(wrappedEvent);
      }
      for (let option of this._preconditionInstruments) {
        if (option.precondition instanceof Function) {
          if (option.precondition.call(this, wrappedEvent, this)) {
            if (option.instruments) {
              for (let instrument of option.instruments) {
                instrument.dispatch(wrappedEvent);
              }
            } else if (option.instrument) {
              option.instrument.dispatch(wrappedEvent);
            }
          }
  ...
  checkContainer() {
    if (this._checked)
      return;
    this._container = d3_exports(this._root.node().ownerSVGElement);
    this._checked = true;
  }
  getObjects() {
  ...
  onObject(pointer2) {
    const element = document.elementFromPoint(pointer2.x, pointer2.y);
    return this._root.node().contains(element) && !element.classList.contains("ig-layer-background");
  }
  query(selector) {
  ...
  pick(shape) {
    ...
    if (shape.type === "rect") {
      const svg2 = this._container.node();
      const rect = svg2.createSVGRect();
      rect.x = shape.x;
      rect.y = shape.y;
      rect.width = shape.width;
      rect.height = shape.height;
      result = [...svg2.getIntersectionList(rect, this._root.node())];
    } else if (shape.type === "point") {
      const elements = document.elementsFromPoint(shape.x + selfBBox.left, shape.y + selfBBox.top);
      result = [...this.getObjects()].filter((obj) => elements.includes(obj));
    }
    return result;
  }
  find(dataFilter) {
...
function collect() {
  ...
  instanceLayers.forEach((layer) => context.layers.set(layer, collectLayer(layer)));
  instanceInstruments.forEach((instrument) => context.instruments.set(instrument, collectInstrument(instrument)));
  instanceSelectionManagers.forEach((selectionManager) => context.selectionManagers.set(selectionManager, collectSelectionManager(selectionManager)));
  instanceInteractors.forEach((interactor) => context.interactors.set(interactor, collectInteractor(interactor)));
...
function restore(context) {
  context.layers.entries().forEach(([obj, context2]) => restoreLayer(obj, context2));
  context.instruments.entries().forEach(([obj, context2]) => restoreInstrument(obj, context2));
  context.selectionManagers.entries().forEach(([obj, context2]) => restoreSelectionManager(obj, context2));
  context.interactors.entries().forEach(([obj, context2]) => restoreInteractor(obj, context2));
}
...
function collectSelectionManager(selectionManager) {
  const keys = Object.keys(selectionManager).filter((key) => !(selectionManager[key] instanceof Function) && !["layers", "result"].includes(key));
  return deepClone(Object.fromEntries(keys.map((key) => [key, selectionManager[key]])));
}
...
function restoreSelectionManager(context, selectionManager) {
  Object.keys(context).forEach((key) => selectionManager[key] = deepClone(context[key]));
}
var contexts = [];
var pointer = 0;
var history_default = {
...
var Interactor = class {
  ...
  _emit(type2, payload) {
    const rawType = type2;
    if (this._rename[type2]) {
      type2 = this._rename[type2];
    }
    ["Command", "Feedback"].forEach((action) => {
      this._listeners[type2 + action].list().forEach((listener) => {
        if (listener instanceof Function) {
          listener(payload);
        }
      });
      if (action === "Command" && this._listeners[type2 + action].size()) {
        if (rawType === "frame" || rawType === "$running") {
          history_default.replace();
        } else {
          history_default.push();
        }
      }
    });
  }
  _wrapEvent(event) {
    ...
    switch (event.type) {
      ...
      default:
        ...
        Object.entries(event.emit || {}).forEach(([k2, v]) => maybeCustomEvent[k2] = v);
  ...
  clone() {
    const option = this._toTemplate();
    const interactor = new this.constructor(this.name, ...option.extraParams || []);
    ...
    if (option.middlewares)
      interactor.appendMiddlewares(...option.middlewares);
    return interactor;
  }
  dispatch(event) {
    ...
    Object.keys(trans).find((key) => {
      if (this[`_${key}Actions`].includes(eventType)) {
        ...
        trans[key][1].forEach((type2) => this._emit(type2, event));
        return true;
      }
      return false;
    });
  ...
  getActions() {
    return this._startActions.concat(this._runningActions).concat(this._outsideActions).concat(this._stopActions).concat(this._abortActions).concat(this._backInsideActions);
  }
  getBaseResponse() {
    return [
      "active",
      ...
      "$abort",
      "$backInside"
    ].map((type2) => this._rename[type2] ? this._rename[type2] : type2);
  }
  getCommands() {
    const base = this.getBaseResponse();
    return base.map((type2) => type2 + "Command");
  }
  getFeedbacks() {
    const base = this.getBaseResponse();
    return base.map((type2) => type2 + "Feedback");
  }
  prependMiddlewares(...middleware2) {
    this._middleware.unshift(...middleware2.filter((x4) => x4 instanceof Function));
  }
  appendMiddlewares(...middleware2) {
    this._middleware.push(...middleware2.filter((x4) => x4 instanceof Function));
  }
  clearMiddlewares() {
    this._middleware = [];
  }
  set startActions(eventName) {
    if (typeof eventName === "string" && !this._startActions.includes(eventName)) {
  ...
  set runningActions(eventName) {
    if (typeof eventName === "string" && !this._runningActions.includes(eventName)) {
  ...
  set outsideActions(eventName) {
    if (typeof eventName === "string" && !this._outsideActions.includes(eventName)) {
  ...
  set stopActions(eventName) {
    if (typeof eventName === "string" && !this._stopActions.includes(eventName)) {
  ...
  set abortActions(eventName) {
    if (typeof eventName === "string" && !this._abortActions.includes(eventName)) {
  ...
  set backInsideActions(eventName) {
    if (typeof eventName === "string" && !this._backInsideActions.includes(eventName)) {
...
Interactor.register = function register4(name, optionOrSelectionManager) {
  let option;
  if (optionOrSelectionManager instanceof Interactor) {
    ...
    option = optionOrSelectionManager;
    option.constructor = option.hasOwnProperty("constructor") ? option.constructor : Interactor;
  }
  registeredInteractors[name] = option;
  return true;
};
Interactor.unregister = function unregister4(name) {
  delete registeredInteractors[name];
  return true;
};
Interactor.initialize = function initialize4(name, ...params) {
  let option;
  if (option = registeredInteractors[name]) {
    const interactor = new option.constructor(name, Object.assign({}, (option.extraParams || [{}])[0] || {}, option.rename || {}), ...(option.extraParams || []).slice(1), ...params);
    for (let actionKey of Object.keys(option)) {
      if (actionKey.endsWith("Actions")) {
        interactor[actionKey] = option[actionKey];
      }
