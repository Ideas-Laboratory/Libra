 ...
 * @returns 
 */
function renderHistogram(root, width, height, data, key, extents, listeners) {
  // layout
  ...
  /*****  add interaction *******/
  const brushGroup = root.append("g")
  ...
  function onBrush({ selection }) {
    ...
    extents.set(key, selection.map(scaleX.invert));
